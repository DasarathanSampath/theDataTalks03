...
<script>
import BlogCards from '~/components/blogCardLayoutEconomics.vue'
import blogsEn from '~/docs/en/economics/blogsEn.js'
import blogsTa from '~/docs/ta/economics/blogsTa.js'
...
export default {
  ...
  async asyncData ({ route, params, store }) {
      ...
      const page = await import(`~/docs/${store.state.locale}/economics.md`)
      const fileContent = fm(page.default)
      const attributes = fileContent.attributes
      async function asyncImport (blogName) {
        const wholeMD = await import(`~/docs/${store.state.locale}/economics/${blogName}.md`)
