...
import blogsEn from '~/docs/en/politics/blogsEn.js'
import blogsTa from '~/docs/ta/politics/blogsTa.js'
...
export default {
  ...
  async asyncData ({ route, params, store }) {
      ...
      const page = await import(`~/docs/${store.state.locale}/politics.md`)
      const fileContent = fm(page.default)
      const attributes = fileContent.attributes
      async function asyncImport (blogName) {
        const wholeMD = await import(`~/docs/${store.state.locale}/politics/${blogName}.md`)
